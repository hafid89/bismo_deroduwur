...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($kategori) || empty($teks)) {
        ...
        if ($stmt->execute([$kategori, $teks, $denda ?: null])) {
            $success = 'Peraturan berhasil ditambahkan!';
        } else {
            $error = 'Gagal menambahkan peraturan';
        }
    }
}
?>
        ...
        <div class="p-8">
            ...
            <form method="POST" action="" class="bg-white rounded-xl shadow-lg p-8">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="teks" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-[#2F5233] focus:outline-none focus:ring-2 focus:ring-[#2F5233]" required>
                </div>
                <div class="mb-6">
                    <label class="block text-gray-700 font-semibold mb-2">Denda (Opsional)</label>
                    <input type="text" name="denda" placeholder="Contoh: Rp. 1.025.000" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-[#2F5233] focus:outline-none focus:ring-2 focus:ring-[#2F5233]">
